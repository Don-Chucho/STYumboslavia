...
public class ControlMenuSTYGui implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource() == this.vistaMenSTY.jbtn_listarTarjetas){
            ...
            String tarjetasPropiedad="";
            if(this.listaTarjetasPropiedad.size()>0){                
                for(int i=0; i<this.listaTarjetasPropiedad.size(); i++){
                    tarjetasPropiedad += "****** Tarjeta de Propiedad No."+(i+1)+"******\n";
                    tarjetasPropiedad += "________Vehículo________\n";
                    tarjetasPropiedad += this.listaTarjetasPropiedad.get(i).getVehiculo()+"\n";
                    tarjetasPropiedad += "________Propietario________\n";
                    tarjetasPropiedad += this.listaTarjetasPropiedad.get(i).getPropietario()+"\n";
                    tarjetasPropiedad += "Código: "+this.listaTarjetasPropiedad.get(i).getCodigo()+"\n";
                    tarjetasPropiedad += "Fecha Expedición: "+this.listaTarjetasPropiedad.get(i).getFechaExp()+"\n";
                    tarjetasPropiedad += "--------------------------------------------------------------------------------------------------\n";                    
                }
                JOptionPane.showMessageDialog(vistaMenSTY, tarjetasPropiedad);
            }else{            
                System.out.println("No se han ingresado Tarjetas de Propiedad");
            }
        }
    }
